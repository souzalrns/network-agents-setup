...
|---|---|---|---|---|
| **S15a** | ~~**Rotação de credenciais incompleta no GitHub Actions do `agent-network-mcp`.**~~ — **DONE em 2026-09-20.** Secret `SUPABASE_SERVICE_ROLE_KEY` actualizada pelo utilizador no GitHub Actions de `souzalrns/agent-network-mcp`. **Re-teste real confirmado:** `transcribe.yml` re-disparado para o reel `Dcg6uPDSoK6` (o mesmo que falhou com 401 no run #105 antes da rotação) — **run #105 (re-run) passou, Success, 57s.** Verificado directamente na tabela `public.transcripts` (Supabase, projecto `agent-network-memory`, via MCP `execute_sql`): 1 registo novo, `url=https://www.instagram.com/reel/Dcg6uPDSoK6/`, `created_at=2026-09-20 23:48:32 UTC`, `transcript` com 1142 caracteres de conteúdo real em português (não vazio, não truncado a zero) — confirma que a gravação no Supabase teve sucesso de facto, não só o job em verde. O aviso "Failed to save: /usr/bin/tar failed with exit code 2" nas Annotations do run **não bloqueou a gravação** — é um warning de cache do `actions/cache@v4` (achado registado à parte, ver **S28**). Dos outros 6 workflows `workflow_dispatch` afectados pela mesma secret (`diagnostico-playwright-portal.yml`, `extrair-imagem.yml`, `scrape.yml`, `testar-portal-justica.yml`, `visual-review.yml`), só `transcribe.yml` foi re-testado nesta sessão — os restantes presumem-se corrigidos pela mesma actualização de secret, mas não foram disparados individualmente. | 15-30 min | Todos os workflows Supabase de `agent-network-mcp` | Novo (2026-09-20, achado durante rotação de credenciais) — **Done** |
...
| **S27** | **Oracle Always Free — limites de recursos A1 reduzidos.** Aviso recebido do utilizador da Oracle Cloud: o tier "Always Free" A1 (ARM) mudou de **4 OCPU / 24GB RAM** para **2 OCPU / 12GB RAM**. A VM `bridge-worker-oracle` (S17, IP `130.61.213.226`) pode estar acima do novo limite ou sujeita a ajuste automático/cobrança. Requer: confirmar a configuração actual da VM face ao novo tier, decidir se é preciso redimensionar (reduzir specs) para continuar no Always Free, ou aceitar custo se ficar acima do limite gratuito. **Não verificado nesta sessão** (achado reportado pelo utilizador, sem confirmação directa na consola Oracle). | A confirmar (depende da config. actual da VM) | S19 (uso da VM para D3/D4) | Novo (2026-09-20, aviso reportado pelo utilizador) |
| **S34** | ~~**Bug real confirmado: `resolve_skill_path()` sempre chamado com `vertical="marketing"` hardcoded**~~ — **DONE em 2026-09-20.** `runner/plan_runner/executor.py:67-68` corrigido: `vertical = step.raw.get("vertical") or "marketing"` (aditivo, mesmo padrão do bloco `knowledge:` — opt-in por step via `step.raw`, sem tocar em `models.py`/`Step`). `docs/orchestration/design/templates/examples/design-flow-demo.plan.yaml` actualizado com `vertical: design` nos 4 steps que precisam (`ux`/`ui`/`copy_ui`/`critic`) — o `hitl` (acção `plan_approve`) não tem skill em nenhum vertical, fica sem alteração. **Confirmado end-to-end (não só por teste isolado):** `python -m plan_runner run design-flow-demo.plan.yaml --mode external --out ...` — `request.json` do step `ux` agora tem `"skill_path": "skills/design/ux_flow/SKILL.md"` (antes: `null`) e `pending_steps/ux/SKILL.md` é escrito com o conteúdo real da skill. Achado extra confirmador: a própria `skills/design/ux_flow/SKILL.md` já tinha `vertical: design` no seu frontmatter (linha do YAML) — o desenho sempre pretendeu isto, só a leitura nunca foi feita. 3 testes novos: `test_skills.py` (+1, `resolve_skill_path` com vertical explícito, e confirma que o vertical errado continua a falhar — não é no-op) + `test_executor.py` (+2, regressão do bug real via `execute_external_request` com `step.raw={"vertical": "design"}`, e não-regressão confirmando que planos sem `vertical:` continuam a resolver contra `marketing`). Suite completa Python: **188/188** (subiu de 185, inclui `-m ""` para os testes lentos). Suite TS: 188/188, inalterada (fix é só Python). | 1-2h | Resolução correcta de skills fora de marketing | Novo (2026-09-20, achado durante auditoria de funcionamento real) — **Done** |
...
| **S29** | **L4 (memória semântica: `remember`/`recall`/`forget`) sem qualquer implementação.** Grep exaustivo em `runner/`+`packages/` confirma zero código — existe só como JSON Schema conceptual em `docs/architecture/memory/contracts.md:7-68`. É o gap central por trás da percepção de "memória entre sessões não funciona" (auditoria de funcionamento real, 2026-09-20). Opções de fecho (não decididas): implementação própria em Python (mais barata, sem infra nova) vs. extrair padrões do `dsh-long-memory` (DeepSeek Harness) — ver `FASE4-SINTESE.md` e `AUDIT-FUNCIONAMENTO-REAL-2026-09-20.md`. | 2-4 dias (implementação própria) | Memória real entre sessões | Novo (2026-09-20, achado durante auditoria de funcionamento real) |
| **S30** | ~~**`working_memory.py` (MEMORY.md por cliente) nunca ligado ao motor — S7b nunca foi feito.**~~ — **DONE em 2026-09-20.** `engine.py`/`langgraph_engine.py` ganharam `_load_client_memory(out, plan, log, run_id)` (novo, em `engine.py`, importado por `langgraph_engine.py`): lê `plan.raw.get("client_id")` (aditivo — planos sem esse campo ficam 100% inalterados), chama `working_memory.read_memory()` (continua read-only, sem função de escrita nova), escreve `out/client_memory.md` uma vez no arranque do run e regista `client_memory_loaded`/`client_memory_absent` em `events.jsonl`. `executor.py::execute_external_request` copia esse ficheiro para `pending_steps/<id>/CLIENT_MEMORY.md` (ao lado de `SKILL.md`/`AGENT.md`) e referencia-o em `request.json["client_memory_path"]` — é isto que faz o contexto "aparecer no step" para o worker externo. **Testado de facto (não só isolado):** `test_client_memory_wiring.py` (4 testes novos, via CLI real): memória injectada e visível em `pending_steps/only/CLIENT_MEMORY.md` + `request.json`; `client_id` sem `MEMORY.md` correspondente regista `client_memory_absent` sem rebentar; plano sem `client_id` fica exactamente igual (0 menções a `client_memory` no `events.jsonl`). Suite completa Python: **192/192** (188→192, 158 rápidos + 34 lentos, ambos 0 falhas). Suite TS: 188/188, inalterada (fix é só Python). | 2-4h | Memória curada a ser usada de facto | Novo (2026-09-20, achado durante auditoria de funcionamento real) — **Done** |
| **S31** | ~~**`session_search.py` (B11/M1) nunca ligado a nada além do seu próprio CLI.**~~ — **DONE em 2026-09-20.** Escolhida a Opção A do brief (mais simples): novo subcomando `search` em `cli.py` — `python -m plan_runner search <run_dir> "<query>"`, ao lado de `run`/`resume`/`compile-graph`. Chama `session_search.search()` directamente (auto-indexa se `events_fts.db` ainda não existir, comportamento que já era da função). Não tocado: `session_search.py` em si (0 alterações), nem `mcp_plan_runner/` (Opção B, tool MCP, não escolhida). **Continua a ser pesquisa por-run** (um `run_dir` de cada vez), não um índice global entre sessões — isso ficaria para um trabalho maior, fora do escopo pedido aqui. 3 testes novos (`test_cli_search.py`, via CLI real, mesmo padrão de fixtures do `test_session_search.py` — `EventLog.append()` real, tokens sem hífen): encontra o evento certo; sem match devolve `hits: []`; auto-indexa sem `build_index()` prévio. Suite completa Python: **195/195** (161 rápidos + 34 lentos, dois runs separados, 0 falhas). | 2-4h | Consulta real de eventos passados | Novo (2026-09-20, achado durante auditoria de funcionamento real) — **Done** |
| **S33** | ~~**Tools (`ToolRegistry`/`ToolExecutor`) nunca chegam a um LLM real.**~~ — **DONE em 2026-09-20.** Fase 1 revelou um achado maior que o esperado: `apps/api/src/index.ts:44-52` **já construía um `ToolRegistry` real** (5 tool sets: database/filesystem/web/brazilian-law/portuguese-law) e até o logava ("MCP Tools loaded: N") — mas nunca o passava ao `Executor` (linha 75, só 4 argumentos). O fio estava literalmente ali, só por ligar. Decisão caso a caso (per instrução, não substituição cega): **Executor** ganhou o tratamento real (é onde as tools se executam); **Planner não foi tocado** — usa um padrão "JSON-in-prompt" (regex `/\{[\s\S]*\}/` sobre texto livre) para decidir agentes/passos, não function-calling, e forçar `chatWithTools()` aí exigiria redesenhar o prompt inteiro em torno de uma tool `create_plan(...)`, fora de proporção para este pedido — documentado, não silenciado. Implementado: `PlanStep.toolsAllowed?: string[]` (novo, opt-in, `packages/shared`); `Executor` construtor ganha 5º parâmetro opcional `toolRegistry?: ToolRegistry` (não-regressão: construtores de 4 argumentos continuam válidos); `executeStep()` só entra no caminho de tools se `step.toolsAllowed` E `toolRegistry` estiverem presentes — 1 volta real de function-calling (`chatWithTools()` → se `toolCalls`, executa cada um via `ToolExecutor.executeTool()` de facto, com `caller: step.agentId` — herda auth/rate-limit/audit do C1/A8/S11 — → `chat()` final sintetiza a resposta). **Bug adicional encontrado e corrigido ao testar** (nunca antes exercitado em produção): `OpenAIProvider.chat()`/`chatWithTools()` descartavam `tool_call_id` e `tool_calls` no mapeamento de mensagens — a API da OpenAI teria rejeitado (400) qualquer resposta `role:'tool'` real. Corrigido com `toOpenAiMessage()` (função partilhada) + `ChatMessage.toolCalls?: any[]` (novo campo). `apps/api/src/index.ts:75` agora passa o `toolRegistry` já construído ao `Executor`. 13 testes novos: `Executor.test.ts` (+4 — não-regressão sem `toolsAllowed`; não-regressão sem `ToolRegistry` no construtor; tool real executada de facto + resultado correcto na 2ª chamada; sem tool-call pedido não gasta 2ª chamada) + `OpenAIProvider.test.ts` (novo ficheiro, 4 testes — nunca tinha teste próprio: tools+tool_choice enviados; `tool_call_id`/`tool_calls` preservados no mapeamento; `toolCalls` parseados da resposta real). Suite completa TS: **196/196** (subiu de 188). Cobertura: `Executor.ts` subiu para 71.03%, `LLMService.ts` para 77.82%. | 0.5-1 dia | Agentes com tools reais em runtime | Novo (2026-09-20, achado durante auditoria de funcionamento real) — **Done** |
...
| ID | Item | Esforço | Bloqueia | Origem |
|---|---|---|---|---|
| **S32** | **Reconhecimento de agentes entre pares não existe, por desenho confirmado (`AGENTS.md:15`: "Personas não invocam outras personas; só o plan/meta-skill/humano orquestra").** Registado como decisão a confirmar/reconsiderar, não bug — o registry (`AgentFactory`) funciona bem para o Orchestrator decidir routing antes de invocar um agente, mas nenhum agente em execução pode consultar quem mais existe. Se o utilizador quiser auto-descoberta real entre agentes, é uma mudança de arquitectura deliberada, não uma correcção. | A definir (depende da decisão) | — | Novo (2026-09-20, achado durante auditoria de funcionamento real) |
| **S35** | **Nenhum catálogo de skills gerado** (`docs/generated/` só tem `AGENTS.md`+`CODE_MAP.md`, sem `SKILLS.md`). A única "descoberta" de skills é a convenção Markdown `skills/meta/using-agent-skills/SKILL.md`, que se auto-declara Advisory ("nada impede um agente de ignorar o mapa de invocação") — zero imposição em código, zero parser de frontmatter. | 1 dia (gerador `docs:skills`, análogo ao `docs:agents` do F6) | Descoberta real de skills | Novo (2026-09-20, achado durante auditoria de funcionamento real) |
| **M1** | *(Nota 2026-09-20: construído e testado, mas nunca ligado ao motor nem a nenhuma tool — ver **S31**.)* ~~**Session search** (FTS5 sobre `events.jsonl`)~~ — **DONE em 2026-09-20.** `runner/plan_runner/session_search.py` (novo, aditivo — não toca em `events.py`): `build_index(run_dir)` lê `events.jsonl` e (re)constrói `events_fts.db` (SQLite FTS5, tabela virtual sobre `payload`, com `event_id`/`run_id`/`type`/`at` como colunas auxiliares `UNINDEXED`); `search(run_dir, query)` procura no `payload` (sintaxe FTS5), auto-indexando se o índice ainda não existir. CLI standalone (`python -m plan_runner.session_search <run_dir> [--search termo]`). 7 testes novos (`test_session_search.py`): índice criado; busca só devolve o evento certo sem falsos positivos; busca por termo presente em 2 eventos devolve os 2; termo ausente devolve vazio; busca sem índice prévio auto-indexa; reindexação após novo evento é idempotente; `events.jsonl` em falta produz índice vazio (não rebenta). | 4h | Nada | Nossa — **Done** |
