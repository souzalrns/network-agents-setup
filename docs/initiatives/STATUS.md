...
| **G5** | ~~Corrigir **2 achados não resolvidos** do `MCP-MAPPING.md`~~ — **DONE, confirmado em 2026-09-19 (B14/D10).** `MCP-MAPPING.md` já tinha as 2 correcções aplicadas ("Correcção G5" no próprio texto): `apps-produto` já está na secção 3.5, `hvac` já reclassificado, contagem já corrigida para 12. Esta linha só não tinha sido actualizada — mesmo padrão do C6/A1/A21/A20. Ver `EXECUTION-PROMPTS.md` C7. | 1h | Integridade do mapeamento | MCP-MAPPING — **Done** |
