...
| **B15** | **Actualizar versão do pnpm no `package.json` (raiz).** Hoje fixa `pnpm@8.0.0`, incompatível com Node 24 (`ERR_INVALID_THIS` ao fazer fetch ao registry — confirmado durante o A16, ao instalar `express-rate-limit`). Subir para `8.15.9` (mesma série major — testado nesta sessão, corrige o bug sem alterar `lockfileVersion`). **Não subir para 9.x sem revisão do `lockfileVersion`** — testado também nesta sessão: `pnpm@9` reescreve o lockfile inteiro de `v6.0` para `v9.0` (2903 linhas de diff), incompatível com o `--frozen-lockfile` do CI enquanto este não for actualizado a par. **Nota:** só registado, `package.json` (raiz) **não foi alterado** nesta sessão — os installs desta sessão usaram `corepack pnpm@8.15.9` ad-hoc, sem tocar no ficheiro; subir a versão fixada é decisão à parte. | 15 min | Nada | Novo (2026-09-20, achado durante A16) |
