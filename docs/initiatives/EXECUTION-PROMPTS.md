...
> **Status em 2026-09-20: DONE (com S11 junto, mesma sessão).** Fase 1 confirmou o achado tal como descrito — zero linhas de `Bearer`/`Authorization`/`auth` em `MCPServer.ts` (47 linhas) e `MCPClient.ts` (55 linhas, não 53 como `AUDIT-TOOLS-MCP.md` citava — pequena divergência de contagem, provavelmente por edição posterior do A18). **Discrepância importante face ao enquadramento inicial desta tarefa (que presumia A8 já feito):** confirmado por grep exaustivo que A8 nunca tinha sido executado. Achado adicional fora do pedido: `createHttpHandler()` nunca é montado em `apps/api` nem em nenhum outro sítio do repo (grep repo-wide) — o subsistema `MCPServer`/`MCPClient`/`ToolExecutor` está desligado de qualquer servidor real, sem superfície HTTP viva em produção hoje. Implementado `packages/mcp/src/server/McpAuth.ts` (novo, 90 linhas): `Authorization: Bearer <key>` contra `MCP_SERVER_KEYS` (lista separada por vírgulas, não uma só — Opção B do brief de S11: caller = hash sha256 de 8 chars da própria key, distinto por key real, não uma declaração não-autenticada), fail-closed (503 sem chave configurada e sem `MCP_ALLOW_UNAUTHENTICATED`, mesmo padrão de `assertAuthConfig`). `MCPServer.createHttpHandler()` autentica antes de `getTools`/`executeTool` e propaga `{caller}` real ao `ToolExecutor` — ver **S11** para o detalhe completo (context/authorize/rateLimit) e a ressalva de que `authorize()` ainda não usa `caller` na decisão. `MCPClient` ganhou `apiKey` no construtor, envia o Bearer. 10 testes novos (`MCPServer.test.ts` ×7, `MCPClient.test.ts` ×3). Suite completa: 188/188 (subiu de 178). Cobertura `MCPServer.ts`: 0%→77.58%. Ver `STATUS.md` S11, `AUDIT-TOOLS-MCP.md` secção 4.
