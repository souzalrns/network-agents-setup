...
|---|---|---|---|
| **Delegation Graph** | Schema + implementação que regista quem delegou o quê a quem, com que âmbito e até quando — para que uma cadeia de delegação tenha um tecto de confiança rastreável (um agente delegado não pode exceder as permissões de quem delegou) | Médio — schema é rápido, a implementação de validação da cadeia é o grosso do trabalho | AgentMesh (identidade dos agentes na cadeia), TrustManager (níveis de confiança de cada nó) |
