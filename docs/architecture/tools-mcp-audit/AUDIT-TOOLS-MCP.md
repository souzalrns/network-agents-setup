...
> **RESOLVIDO em 2026-09-19, com correcção à recomendação acima: `ssrf-req-filter` não é utilizável aqui.** Devolve um `http.Agent` clássico; o `fetch` global do Node (undici) não aceita `agent`, só `dispatcher` — confirmado empiricamente que o agent nunca é invocado. A guarda real (`packages/mcp/src/tools/built-in/SsrfGuard.ts`) replica o mesmo princípio (IP pós-DNS, fecha a janela de rebinding) mas ao nível do `connect` do `undici.Agent`, via `buildConnector`. `http_request` passa a usar `fetch` do `undici` explicitamente. Testado: 18 testes novos (`SsrfGuard.test.ts` + `web.test.ts`), suite completa 124/124. Ver `EXECUTION-PROMPTS.md` A5.**
...
> **RESOLVIDO em 2026-09-20 (A8/S11) — `MCPServer.ts` e `MCPClient.ts` propagam caller real.** Achado prévio à correcção: apesar de `GOV-FASE1.md` §2b já citar A8 como pendente, um grep exaustivo por `Bearer`/`Authorization`/`auth` nos 2 ficheiros confirmou **zero linhas de autenticação existentes** — A8 nunca tinha sido de facto executado (discrepância com o enquadramento inicial desta tarefa, que presumia o contrário). Achado adicional, fora do pedido: `MCPServer`/`MCPClient`/`ToolExecutor` **não estão ligados a nenhum servidor real** — `createHttpHandler()` não é montado em `apps/api` nem em nenhum outro sítio do repo (grep repo-wide, único resultado é a própria definição); hoje é um subsistema construído e testado isoladamente, sem superfície de ataque HTTP real em produção. A correcção fica pronta para quando for ligado. Implementação: `packages/mcp/src/server/McpAuth.ts` (novo) — `authenticateRequest()` exige `Authorization: Bearer <key>`, fail-closed (mesmo padrão de `apps/api/src/middleware/auth.ts:assertAuthConfig()`), `MCP_SERVER_KEYS` (lista separada por vírgulas — múltiplas keys, não uma só, para o caller ser distinguível de facto e não só "pronto para o futuro"); `callerIdFromKey()` deriva um hash sha256 de 8 chars da própria key (Opção B recomendada: caller autenticado por posse da chave, não uma declaração `X-Caller-Id` não verificada). `MCPServer.createHttpHandler()` autentica antes de despachar `getTools`/`executeTool` e passa `{caller}` como `ExecutionContext` real ao `executor.executeTool()` (antes: chamado sem 3º argumento nenhum, sempre `caller='unknown'`). `MCPClient` ganhou um `apiKey` no construtor (opcional, lê `MCP_CLIENT_KEY` por omissão) e envia `Authorization: Bearer <key>` em `connect()`/`executeTool()`. **Ressalva importante (auto-crítica):** `ToolPolicy.authorize()` **não usa `caller` na decisão de allow/deny** — só `providedKey`/`toolName`/`profile` decidem; `caller` só entra no audit trail, no `ActionReceipt.actor` e no bucket do `rateLimit()` (que passa a ser per-caller de facto, não só per-'unknown' partilhado). Isto fecha o "authorize() sem sujeito" do enquadramento original de forma parcial: o sujeito agora existe e é real (autenticado por posse de key), mas não altera ainda a decisão de autorização em si — isso exigiria SCOPES por caller, fora do escopo pedido aqui. 10 testes novos: `tests/unit/MCPServer.test.ts` (7 — 2 keys geram callers diferentes; sem header nega 401 sem invocar o executor; key errada nega 401; sem `MCP_SERVER_KEYS` nega 503 fail-closed; `getTools` também exige auth; não-regressão do método público de 2 argumentos) + `tests/unit/MCPClient.test.ts` (3 — `connect()`/`executeTool()` enviam o Bearer; sem key não envia header, não regride A18). Suite completa: 188/188 (subiu de 178). Cobertura de `MCPServer.ts`: **0% → 77.58%** statements (era o achado do S24). Ver `EXECUTION-PROMPTS.md` A8/S11 e `TEST-COVERAGE.md`.**
